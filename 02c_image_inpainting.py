...
def create_mask(h, w, type='box', box_scale=0.25):
    """
    Creates a binary mask (1 = Valid, 0 = Missing).
    Returns shape (H, W, 1).
    """
    ...
    if type == 'box':
        cy, cx = h // 2, w // 2
        sy, sx = int(h * box_scale), int(w * box_scale)
        mask[cy - sy // 2 : cy + sy // 2, cx - sx // 2 : cx + sx // 2, :] = 0.0

    elif type == 'random':
        mask = (torch.rand((h, w, 1)) > 0.8).float()

    return mask


# ------------------------------------------------------------------ #
#  Core inpainting routine                                            #
# ------------------------------------------------------------------ #

def image_inpainting(image, cfg, out_dir: str, mask_type='box'):
    ...
    mask_2d = create_mask(image.h, image.w, type=mask_type)
    mask_flat = mask_2d.reshape(1, -1, 1).cuda()
    masked_gt = ground_truth * mask_flat
    ...
    for step in tqdm(range(cfg.train.steps), desc=f"Inpainting {cfg.net.model_name}..."):

        model_output, _ = inr(model_input)

        if cfg.net.model_name != 'bacon':
            diff = (model_output - ground_truth) * mask_flat
            loss = (diff ** 2).sum() / mask_flat.sum()
        else:
            loss = 0
            for output in model_output:
                diff = (output - ground_truth) * mask_flat
                loss += (diff ** 2).sum() / mask_flat.sum()
    ...
    save_image_flat(masked_gt, image.h, image.w, image.ch,
                       os.path.join(model_dir, "masked_input.png"))
    ...
    axes[1].set_title(f"Inpainted Spectrum (s-PSNR: {spectrum_psnr:.3f} dB)")
    axes[1].axis('off')
    plt.colorbar(im2, ax=axes[1], fraction=0.046, pad=0.04)

    plt.tight_layout()
    plt.savefig(os.path.join(model_dir, "spectrum_inpainting_2d.png"),
    ...
    plt.title(f"Inpainting Dynamic {cfg.net.model_name} (PSNR: {final_psnr:.2f})")
    ...
    axes[0].imshow((masked_gt.cpu().view(image.h, image.w, image.ch).detach().numpy() + 1.0) / 2.0,
                   cmap='gray')
    axes[0].set_title(f"Masked Input ({mask_type})")

    axes[1].imshow((final_output.cpu().view(image.h, image.w, image.ch).detach().numpy() + 1.0) / 2.0,
                   cmap='gray')
    axes[1].set_title(f"Inpainted Output (PSNR: {final_psnr:.2f} dB)")

    axes[2].imshow((ground_truth.cpu().view(image.h, image.w, image.ch).detach().numpy() + 1.0) / 2.0,
                   cmap='gray')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(model_dir, "image_inpainting.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Inpaint a masked 2D image with an INR.")
    ...
    out_dir = make_output_dir(path_stem(cfg.data.image_path), "image_inpainting")
    ...
    for model_name in model_names:
        ...
        for seed in seeds:
            ...
            metrics = image_inpainting(image, cfg, out_dir, mask_type='random')
