...
def image_fitting(image, cfg, out_dir: str):
    ...
    axes[0].set_title(f"Ground-truth Spectrum (Log)")
    axes[0].axis('off')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(model_dir, "spectrum_fitting_2d.png"),
    ...
    axes[1].imshow((final_output.cpu().view(image.h, image.w, image.ch).detach().numpy() + 1.0) / 2.0,
                   cmap='gray')
    axes[1].set_title(f"Fitted Image (PSNR: {final_psnr:.3f} dB)")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(model_dir, "image_fitting.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fit a 2D image with an INR.")
    ...
    out_dir = make_output_dir(path_stem(cfg.data.image_path), "image_fitting")
    ...
    for model_name in model_names:
        ...
        for seed in seeds:
            ...
            metrics = image_fitting(image, cfg, out_dir)
