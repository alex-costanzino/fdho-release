...
def add_noise(image_tensor, sigma=0.1):
    """
    Adds Gaussian noise to the image.
    Assumes image is in range [-1, 1].
    """
...
# ------------------------------------------------------------------ #
#  Core denoising routine                                             #
# ------------------------------------------------------------------ #

def image_denoising(image, cfg, out_dir: str, noise_sigma=0.1):
    ...
    noisy_target = add_noise(ground_truth, sigma=noise_sigma)
    ...
    for step in tqdm(range(cfg.train.steps), desc=f"Denoising {cfg.net.model_name}..."):

        model_output, _ = inr(model_input)

        if cfg.net.model_name != 'bacon':
            loss = ((model_output - noisy_target) ** 2).mean()
        else:
            loss = sum(((out - noisy_target) ** 2).mean() for out in model_output)
    ...
    save_image_flat(noisy_target, image.h, image.w, image.ch,
                       os.path.join(model_dir, "noisy_input.png"))
    ...
    axes[1].set_title(f"Denoised Spectrum (s-PSNR: {spectrum_psnr:.3f} dB)")
    axes[1].axis('off')
    plt.colorbar(im2, ax=axes[1], fraction=0.046, pad=0.04)

    plt.tight_layout()
    plt.savefig(os.path.join(model_dir, "spectrum_denoising_2d.png"),
    ...
    plt.title(f"Denoising Dynamic {cfg.net.model_name} (PSNR: {final_psnr:.2f})")
    ...
    axes[0].imshow((noisy_target.cpu().view(image.h, image.w, image.ch).detach().numpy() + 1.0) / 2.0,
                   cmap='gray')
    axes[0].set_title(f"Noisy Input (σ={noise_sigma:.4f})")

    axes[1].imshow((final_output.cpu().view(image.h, image.w, image.ch).detach().numpy() + 1.0) / 2.0,
                   cmap='gray')
    axes[1].set_title(f"Denoised Output (PSNR: {final_psnr:.3f} dB)")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(model_dir, "image_denoising.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Denoise a 2D image with an INR.")
    ...
    out_dir = make_output_dir(path_stem(cfg.data.image_path), "image_denoising")
    ...
    for model_name in model_names:
        ...
        for seed in seeds:
            ...
            metrics = image_denoising(image, cfg, out_dir, noise_sigma=25.0 / 255.0)
